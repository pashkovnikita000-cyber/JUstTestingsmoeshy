...
- [x] **WALLET-01**: Пользователь может добавить ETH-кошелёк командой `/addwallet <address> <name>`
- [x] **WALLET-02**: Бот валидирует формат ETH-адреса (0x + 40 hex chars)
- [x] **WALLET-03**: Пользователь может посмотреть список своих кошельков командой `/wallets` (имя, адрес, баланс ETH, баланс USD)
- [x] **WALLET-04**: Пользователь может удалить кошелёк командой `/removewallet <address>`
- [x] **WALLET-05**: Каждый пользователь видит только свои кошельки (изоляция по user_id)

### Monitoring (MON)

- [ ] **MON-01**: Фоновый worker проверяет новые транзакции каждые 60 секунд для всех отслеживаемых кошельков
- [x] **MON-02**: Бот шлёт алерт пользователю при обнаружении транзакции > $100 USD (по курсу на момент tx)
- [x] **MON-03**: Алерт содержит: имя кошелька, тип (входящая/исходящая), сумму ETH и USD, tx hash, ссылку на Etherscan
- [x] **MON-04**: Повторные алерты на одну и ту же транзакцию не шлются (дедупликация по tx hash)
